...
- 추적 대상별 공식 채널 목록 작성
- 출처의 RSS, sitemap, API 제공 여부 확인
- 샘플 자료 10~20건으로 문서 템플릿 검증
- 중요도와 관련성 판정 기준 확정

### Phase 1 — 최소 수집기

- Source Registry 정의
- RSS/Atom과 GitHub release 수집 구현
- 체크포인트, URL 정규화, 중복 제거 구현
- 원문 링크 중심의 Inbox 문서 생성
...
1. 자동화 실행 환경: 로컬 PC, GitHub Actions, 별도 서버 중 어디에서 실행할지
2. 구현 언어와 런타임
3. LLM 사용 위치, 모델, 비용 및 민감정보 정책
4. 뉴스 수집에 사용할 허가된 API 또는 RSS 범위
5. Inbox 문서를 사람이 승인하는 구체적인 방식
6. 중요도 판정 기준과 일간 최대 문서 수
7. 원문 수정 또는 삭제 시 기존 노트의 보존 정책

## 11. 첫 번째 검증 작업

각 결제 네트워크에서 공식 블로그 또는 뉴스룸 1개, GitHub 또는 YouTube 1개를 선정한다. 최근 자료를 소량 수집해 다음을 검증한다.

- 안정적으로 신규 항목을 탐지할 수 있는가
- 원문 제목과 링크가 정확히 보존되는가
- 한국어 요약만 읽어도 핵심 변화를 이해할 수 있는가
- 기술 키워드가 실제 학습 노트로 이어질 만큼 구체적인가
- 중복 보도자료와 재배포 뉴스를 하나의 사건으로 묶을 수 있는가

이 검증 결과를 바탕으로 Source Registry와 최종 문서 템플릿을 확정한다.
