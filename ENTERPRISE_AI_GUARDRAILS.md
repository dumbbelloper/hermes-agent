...
Hermes Agent의 현재 자동화 기능은 공개 자료의 목록 메타데이터 수집과 로컬 보존이다. Obsidian 문서 작성은 사람이 검토하는 초기 작업으로 시작했으며 자동 생성은 아직 구현하지 않았다. 현재 구현과 향후 문서 자동화 모두 다음 기준을 적용한다.
...
- 외부 사이트 로그인, 댓글, 이메일, 메시지 전송: G2~G3
- API key, cookie, 유료 API 사용: G2 이상
- 개인정보·결제정보 수집: 기본 제외
- source 전체 복제 또는 저작물 재배포: 기본 제외
- 자동 게시, production 배포, 결제 실행: 현재 scope 밖
